...
- [ ] `docker-compose.yaml` を確認・更新（PostgreSQL + Redis のローカル起動）
- [ ] `.env.example` 作成（必須環境変数の一覧、ローカル用デフォルト値）
- [ ] `apps/api` の Dockerfile 作成（dev / prod 両対応のマルチステージ）
- [ ] `apps/crawler/` ディレクトリ作成・`package.json`・Dockerfile（クローラ + ランキングバッチを兼ねる）
- [ ] `pnpm dev` で web / api / Postgres / Redis が同時起動できることを確認
- [ ] Sentry のローカルダミー DSN 設定（本番接続は Phase 9）
- [ ] `apps/api` に `/healthz` エンドポイント追加
...
- [ ] `apps/crawler/` 初期構成（`package.json` / TypeScript / 共通ロガー）
...
- [ ] AST 正規化ハッシュで重複排除
- [ ] repo 単位の足切り（採用候補 30 個以上で eligible=true、ランダムサンプリング最大 100 個）
...
- [ ] `POST /api/play-sessions/solo` Controller
  - [ ] `eligible=true` repo からランダム 1 選定
...
- [ ] Vercel Env Vars 登録（`NEXT_PUBLIC_API_URL`、`GITHUB_CLIENT_ID/SECRET`、`SENTRY_DSN`、`ADSENSE_PUB_ID`）
...
- [ ] **ECS Cluster 作成**
- [ ] **ECS Task Definition 作成**（api / crawler / batch の 3 種）
- [ ] **ECS Service 作成**（api、Fargate Spot 50% + On-Demand 50% 混成、min 1 / max 10）
- [ ] **ECR リポジトリ作成**（api / crawler / batch 共通）
- [ ] **EventBridge ルール作成**：
  - [ ] 週次月曜 03:00 JST → crawler ECS RunTask
  - [ ] 毎時 00 分 → batch ECS RunTask
  - [ ] 月初 04:00 JST → license-recheck ECS RunTask
- [ ] AWS Secrets Manager に環境変数登録（DATABASE_URL / REDIS_URL / JWT_SECRET / GITHUB_PAT 等）
- [ ] **IAM ロール作成**：ECS タスクロール / タスク実行ロール / GitHub OIDC ロール
- [ ] CloudWatch ロググループ作成（api / crawler / batch）
...
- [ ] `crawler-batch-deploy-ecs` ワークフロー（同上、Scheduled Task 用）
...
- [ ] GitHub ログインが成功する
- [ ] プレイ → スコア保存 → ランキング反映
- [ ] 手動で crawler を ECS RunTask 起動して動作確認
- [ ] 毎時 batch が EventBridge で起動することを確認
...
- [ ] **問題プールのブートストラップ運用**
  ...
  - [ ] 各言語 30〜50 repo の `eligible=true` を確保
