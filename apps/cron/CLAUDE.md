...
実装済みの定期実行タスクは以下の 4 つ。

| コマンド | スケジュール | 用途 |
| --- | --- | --- |
| `pnpm crawler:run:typescript` | 週次（月曜 03:00 JST） | GitHub 上の OSS（TypeScript）から問題プールを収集 |
| `pnpm crawler:run:javascript` | 週次（月曜 04:30 JST） | GitHub 上の OSS（JavaScript）から問題プールを収集 |
| `pnpm crawler:run:go` | 週次（月曜 06:00 JST） | GitHub 上の OSS（Go）から問題プールを収集 |
| `pnpm crawler:license-recheck` | 月初 04:00 JST | 収集済み repo のライセンス再検証（言語非依存） |

ランキング集計は cron では行わない。月間ランキングは v2 で `/finish` 同期 UPSERT（`docs/spec/monthly-ranking/README.md`）に、オールタイムランキングは `user_language_best` のリアルタイム集計（`docs/spec/score-ranking/README.md`）に移行済みで、cron バッチ・snapshot テーブルは持たない。

CLI 名は機能名（crawler）に合わせており、ディレクトリ名（cron）は「全部 cron 駆動」という実行モデルを表す。

**crawler は言語ごとに独立した task** として実装する：AST 抽出層が言語固有で、1 言語の rate limit / 障害を他言語に波及させないため。新言語追加時は `task/crawler-run-<slug>.ts` を新規作成し、`LANGUAGE_SLUG` と `RUN_TYPE = "crawler_<slug>"` をハードコードする（`crawler_runs.runType` で言語識別を区別する）。現時点では TypeScript / JavaScript / Go の 3 言語。

AST 抽出は `ast/language-extractor.ts` の **`LanguageExtractor` strategy** に切り出してあり、`process-repo.ts` は extractor を DI で受け取るだけ（fetch / ライセンス / サンプリング / 保存の共通フローは言語非依存）。実装は 2 つ：

- **`TsFunctionExtractor`**（TypeScript / JavaScript 共有）— TypeScript Compiler API。`.js`/`.mjs`/`.cjs` は拡張子から ScriptKind.JS が自動推定される。
- **`GoFunctionExtractor`**（Go）— `tree-sitter-go`(WASM)。wasm ロードが非同期なので `createGoExtractor()` を task 冒頭で await する。テスト関数名の除外（`Test`/`Benchmark`/`Example`/`Fuzz`）や除外パスパターン（`vendor/`・`_test.go`）は言語固有に持つ。
