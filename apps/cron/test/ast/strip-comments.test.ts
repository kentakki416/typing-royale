...
import { stripComments } from "../../src/ast/strip-comments"

describe("stripComments", () => {
  describe("正常系", () => {
    it("単一行コメントを除去する", () => {
      const result = stripComments("const x = 1 // initial\nconst y = 2\n")
    ...
    it("テンプレートリテラル内の `//` は保護される", () => {
      const result = stripComments("const url = `https://example.com/${id}`\n")
      expect(result).toContain("https://example.com")
    })

    it("正規表現リテラル内の `//` パターンは保護される", () => {
      const result = stripComments("const re = /\\/\\/ comment/g\n")
