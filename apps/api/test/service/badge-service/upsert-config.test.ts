...
const mockUpsert = vi.fn<(_0: number, _1: { displayItems: string[] }) => Promise<BadgeConfigRow>>()
  ...
  describe("正常系", () => {
    it("Repository.upsert に値を渡し戻り値を返す", async () => {
      ...
      const result = await upsertConfig(
        { displayItems: ["grade", "rank"], userId: 42 },
        { badgeConfigRepository: mockRepo },
      )

      expect(result).toEqual(expected)
      expect(mockUpsert).toHaveBeenCalledWith(42, { displayItems: ["grade", "rank"] })
    })
  })
})
