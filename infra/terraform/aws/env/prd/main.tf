...
# 各 workload はこの cluster と role を参照する。
...
# image push 完了後に variable 経由で 1 以上に上げる。
...
# - 専用 ECR (project-template-migration) + 専用 Dockerfile (apps/api/Dockerfile.migration) を使う
