...
> **配色は順位 / グレード / 報酬種別で決まり、言語では変えない**（殿堂入り=順位テーマ 金/銀/銅、月間 TOP10=青固定、昇格=グレード別）。言語は表示ラベル（TS / JS / Go）が変わるだけでデザイン・配色は共通。reward は言語マスタ駆動で、現状の対象は TypeScript / JavaScript / Go（新しい言語をマスタに追加すれば自動的に対象になる）。
