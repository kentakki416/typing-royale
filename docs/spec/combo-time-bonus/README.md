...
- combo 0 に戻った後、再度 combo 30 / 60 / 90 ... に達しても **同じマイルストーンが 2 回発火することはない**（次項参照）

### 発火タイミングは 1 セッション 1 回のみ

各マイルストーン (combo 30 / 60 / 90 / 120 / ...) は **1 セッション中 1 回しか発火しない**。理由：

- 「ミスして combo を意図的にリセットして再度ボーナスを取りに行く」というループを防ぐ
- 取得済みマイルストーンは「次のマイルストーン（次の 30 の倍数）まで再達成」しないと無効

例：
- combo 29 → 30 で「+1 秒」発火（マイルストーン 30 を取得）
- combo 30 → 5 にリセット → combo 32 まで戻る → 発火しない（マイルストーン 30 はもう取得済み）
- combo 60 で「+2 秒」発火（マイルストーン 60 を取得）
