...
- [仕様](#仕様)
  ...
  - [発火タイミングは 1 セッション 1 回のみ](#発火タイミングは-1-セッション-1-回のみ)
  - [HUD 演出](#hud-演出)
  - [効果音](#効果音)
  - [スコアへの影響](#スコアへの影響)
...
| 40 | **+2 秒** |
| 60 | **+3 秒** |
| 80 以降は **20 combo ごと**<br/>(80, 100, 120, 140, ...) | 各 **+3 秒** |

- 1 回の加算は **最大 +3 秒**（+5s や +10s のような派手なジャンプは無し）
- 累積上限なし。理論的には combo を切らさず維持し続ければ無限に延長されるが、現実的には combo 100〜150 程度が上限
- combo 20 未満ではボーナスは発生しない

### combo の維持・喪失と時間ボーナスの関係

- 一度もらった時間ボーナスは **永続**。後でミスして combo が 0 に戻ってもプレイ時間は減らない
- combo 0 に戻った後、再度 combo 20 / 40 / 60 ... に達しても **同じマイルストーンが 2 回発火することはない**（次項参照）

### 発火タイミングは 1 セッション 1 回のみ

各マイルストーン (combo 20 / 40 / 60 / 80 / ...) は **1 セッション中 1 回しか発火しない**。理由：

- 「ミスして combo を意図的にリセットして再度ボーナスを取りに行く」というループを防ぐ
- 取得済みマイルストーンは「次のマイルストーン（次の 20 の倍数）まで再達成」しないと無効

例：
- combo 19 → 20 で「+1 秒」発火（マイルストーン 20 を取得）
- combo 20 → 5 にリセット → combo 22 まで戻る → 発火しない（マイルストーン 20 はもう取得済み）
- combo 40 で「+2 秒」発火（マイルストーン 40 を取得）
...
const detectBonuses = (logs: KeystrokeLogs): BonusEvent[] => {
  ...
  const triggered = new Set<number>()  // 既発火マイルストーン
  let combo = 0
  for (const e of logs) {
    if (e.is_correct) {
      combo += 1
