...
step1〜5 で導入した `@repo/db` / `@repo/logger` / `@repo/errors` / `@repo/config` / `@repo/redis` を `apps/api` の全コードに浸透させ、互換 wrapper と旧パスを完全削除する。`@repo/db` / `@repo/redis` は **factory のみ提供** なので、この step で `apps/api/src/index.ts` を **DI assembly** に書き換えて Repository に client を渡す形に統一する。step1〜5 完了時点で wrapper 経由でも動く状態が維持されていることが前提。
...
2. `@repo/errors` / `@repo/logger` / `@repo/config` / `@repo/redis` / `@repo/api-schema` / `@repo/db` の build
...
ルート `CLAUDE.md` の Project Overview にも `packages/db` / `packages/logger` / `packages/errors` / `packages/config` / `packages/redis` を追記。
...
→ env 検証 → DB 接続 (factory) → Redis 接続 (factory) → ログ出力 → 切断、まで通れば 5 packages の組み合わせが正しく動いていることが確認できる。
...
- 全 import が `@repo/db` / `@repo/logger` / `@repo/errors` / `@repo/config` / `@repo/redis` 経由
