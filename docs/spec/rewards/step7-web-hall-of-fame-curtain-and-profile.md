...
step5 で Hall of Fame の MVP を実装したが、`docs/mocks/hall-of-fame.html` 準拠の **上位 3 名クラウン + クリックでカーテン演出 + 神モーダル** が未反映だった。本 step で (1) 上位 3 名の hof-card レイアウト + クラウン + カーテン演出 + 神モーダル化、(2) 神モーダルに **お気に入りリポジトリ** を表示する `users.favorite_repo_url` カラムと設定 UI を追加、(3) TODO.md の現状反映を 1 PR で行う。
...
| Route | 変更内容 |
|---|---|
| `/hall-of-fame` | テーブル形式 → mock 準拠 hof-card に書き換え。上位 3 名（rank=1〜3）は `.hof-card.has-crown.tappable[data-rank=gold/silver/bronze]` + クラウン SVG。クリックで `.curtain-stage.active` を 1.8 秒再生して `.god-modal` を露出。4 位以下も同じ hof-card 形式（コメント列なし）で下に並べる |
...
GitHub のソーシャルリンクは `entry.user.github_username` を後続 PR で同じスキーマに追加して使う（GitHub OAuth ログイン時の login を保持）。X のソーシャルリンクは MVP では割愛。
...
> **注記**：`display_name` は廃止された（後続 PR で User.displayName ごと撤去）。表示・認証は `github_username` を使う。マイページから display_name を編集する動線も存在しない。
...
2. 上位 3 名は `hof-card has-crown tappable` + `data-rank` で描画、4 位以下も同じ hof-card 形式（コメント列なし）
...
7. モーダル中身: avatar / 表示名 / stat / grade / favorite_repo_url（あれば）/ リプレイリンク
...
- **上位 3 名はクラウン演出、4 位以下は同じ hof-card 形式（クラウンなし）**：mock 通り段差を付ける。コメント機能は廃止のためコメント列は無し
...
`entries[].user` に `favorite_repo_url: z.string().nullable()` を追加。同時に `github_username: z.string().nullable()` も追加された（step4 schema 更新と合わせて）。
...
mock 準拠で **上位 3 名 = hof-card has-crown + クリックで `<CurtainModal>` 開閉**、4 位以下も同じ hof-card 形式（クラウンなし、コメント列なし）。Server Component は引き続き fetch のみ、開閉 state は Client Component に分離。
...
`favorite_repo_url` 入力欄を追加（既存 can_public_ranking と同列。display_name は廃止）。Server Action で patch。
...
| Playwright | (1) /hall-of-fame 上位 3 名カードクリックでカーテン → モーダル表示 / (2) モーダル × でクローズ / (3) /mypage/account で favorite URL 入力 → 再 fetch でモーダルに表示 / (4) 4 位以下も hof-card 形式で表示される |
...
- 本 step で意図的に省略したもの:
  ...
  - 「⚡ この神に挑戦」CTA（指名対戦不可）
  - 4 位以下も hof-card 形式で統一済み（クラウンなし、コメント列なし）
