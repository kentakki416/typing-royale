...
step2 で実装した `GET / PUT /api/user/badge-config` を Web から消費する画面を実装する。マイページのタブ構成（既存：概要 / 特典 / プレイ履歴 / 設定）に「バッジ」を追加し、`/mypage/badge` で表示項目とテーマを編集できるようにする。
...
| Route | コンポーネント | 概要 |
|---|---|---|
| `/mypage/badge` | Server Component + Form Client Component | バッジ表示項目チェックボックス + リアルタイムプレビュー + 保存（背景は黒固定、テーマ選択は廃止） |
...
|---|---|---|---|
| `GET /api/user/badge-config` | ページ表示時 | Server Component → `apiClient.get` | 必須 |
| `GET /api/user` | 同上 | 同上（プレビュー URL に `username` を入れるため） | 必須 |
| `PUT /api/user/badge-config` | 保存ボタン押下 | Server Action → `apiClient.put` | 必須 |
...
|---|---|---|
| step2 (`/api/user/badge-config` GET/PUT) | 設定の取得・保存 | 必須前提 |
...
| 既存 `GET /api/user` | 自分の `github_username` 取得（プレビュー URL に使う） | 流用 |
...
Server Component が初期 `displayItems` / `username` を props で渡し、Client Component（`BadgeForm`）が編集・送信を担当。
...
sequenceDiagram
    ...
    SC->>API: GET /api/user/badge-config + GET /api/user (並列)
    API-->>SC: { display_items, updated_at } / { github_username, ... }
    ...
    U->>Form: 保存ボタン
    Form->>Action: updateBadgeConfigAction({ displayItems })
    Action->>API: PUT /api/user/badge-config
    API-->>Action: 200 { display_items, updated_at }
    Action-->>Form: ok
...
1. Server Component が cookie 付きで `GET /api/user/badge-config` と `GET /api/user` を並列 fetch
2. 取得値を `BadgeForm` Client Component に渡す
3. ユーザーがチェックボックスを操作 → ローカル state を更新、`dirty=true`
...
export function BadgeForm({ initialDisplayItems, username }: Props) {
  ...
  const badgeUrl = `/api/internal/badge-preview/${encodeURIComponent(username)}`
  const previewUrl = `${badgeUrl}?v=${previewKey}`
  const embedSnippet = `<img src="https://typing-royale.example.com/badge/${username}.svg" alt="${username} の Typing Royale バッジ">`
...
export const updateBadgeConfigAction = async (input: { displayItems: string[] }): Promise<GetBadgeConfigResponse> => {
...
| 区分 | 内容 |
|---|---|
| 初回表示 | dev-login で alice → `/mypage/badge` → default `["grade", "best_score"]` がチェック済み（背景は常に黒固定） |
| チェック変更 → 保存 | rank をチェック → 保存 → 「HH:MM:SS に保存しました」表示 + プレビュー更新 |
| バリデーション | 全部チェック外して保存ボタンが disabled になる、6 個超チェックできない |
