...
ゲストプレイ完走から **15 分以内**に「GitHub で記録を残す」CTA 経由でログインすると、直前のスコアが認証済みアカウントに紐付いて DB に保存される（**claim** 機能）。

- ゲスト完走時にサーバーが `/api/play-sessions/guest/finish` のレスポンスに **不透明な `claim_ticket`（UUID）** を含めて返す
- 完走時のサーバー再集計済みデータは Redis に `claim:{ticket}` キーで 15 分 TTL で保存
- クライアントは ticket だけ sessionStorage に保持して OAuth フローを通過
- ログイン後の画面で常設の `GuestClaimWatcher` Client Component が sessionStorage を見て `POST /api/play-sessions/claim` を実行
- claim 成功時は通常 `/finish` と完全同形のレスポンス（順位・グレード等）を返し、リザルト画面相当を再描画

仕様 / 設計の詳細は [`../guest-claim/README.md`](../guest-claim/README.md) を参照。
...
> ゲストプレイ後の `POST /api/play-sessions/claim` は本ドキュメント外で別 feature 化されている。仕様は [`../guest-claim/README.md`](../guest-claim/README.md) を参照。
