...
| `playTimeBonus` | combo マイルストーン（30 / 60 / 90 / ...）到達時 | G5 → C6 → E6 の上昇 arpeggio（note 間隔 0.07s）。combo 維持のごほうび |
| `playUrgentTick` | 残り 10 秒の境界をまたいだ瞬間（残り 30 秒はフラッシュのみで SE なし） | 高音→低音の警告 chirp。1 回だけ鳴る |
...
1. **メカニカルクリック**：25ms の noise buffer を band-pass + ~35ms decay envelope で減衰させ、メカニカルキーボードの「コッ／カチッ」感を作る
...
音量コントロール UI は持たず、OS / ブラウザ側で調整する前提。`MASTER_VOLUME = 0.6` 固定で動的変更 API も localStorage 永続化も存在しない。
...
```
  ...
  各 SE 関数 ──→│ OscillatorNode / BufferSource    │
                ...
                └────────────┬────────────────────┘
                             ...
                             ▼
                      ┌─────────────┐
                      │ masterGain  │  ← MASTER_VOLUME=0.6 を初回 setValueAtTime のみ（動的変更 API 無し）
                      └─────┬───────┘
                             │
                             ▼
...
- 打鍵音は全体で 65ms 以下（クリック層 ~40ms + ピッチアクセント ~65ms）に収め、高速連打時の残響被りを防ぐ
...
現状は `MASTER_VOLUME = 0.6` 固定で動的変更も永続化も行わない。`useSyncExternalStore` / localStorage 連携 / React への通知機構はいずれも存在せず、SE 関数を直接呼び出すだけで完結する。
...
| 画面 | 概要 |
| --- | --- |
| プレイ画面 (`/play/[sessionId]`) | SE の再生は use-typing-engine / play-loop / use-countdown / replay-player から発火する |
...
sequenceDiagram
    ...
    SFX->>SFX: masterGain に MASTER_VOLUME=0.6 を setValueAtTime
    ...
    Note over U,AC: combo マイルストーン到達
    TE->>SFX: playTimeBonus()
    SFX->>AC: G5 → C6 → E6 の上昇 arpeggio（note 間隔 0.07s）
```
