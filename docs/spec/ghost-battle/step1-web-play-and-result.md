...
|---|---|---|---|
| `POST /api/play-sessions/challenge-gods` | 言語選択画面の「神々に挑戦」クリック時 | Server Action `startPlaySession`（mode 引数で `solo` / `challenge_gods` を分岐、既存） | 任意 |
...
- ghost 進捗更新ロジックは `useGhostPlayback` カスタムフックに分離し、PlayLoop 本体の `startAtRef` を共有して `elapsedMs` 同期で消費する構造にする（[ゴースト rAF 再生ロジック](#ゴースト-raf-再生ロジック)）。これにより PlayLoop の rAF tick はキー入力判定だけに集中でき、ghost ロジックを replay-viewer 等で再利用しやすくなる
  ...
  <aside className="col-sidebar">
    <div className="card god-frame">
      ...
      <div className="flex-center gap-12 mb-8">
        <span className="avatar">{(ghostUserDisplay.github_username ?? "??").slice(0, 2).toUpperCase()}</span>
        <div>
      ...
      <div className="modal modal-lg" style={{ position: "relative" }}>
        ...
        <div className="text-center mb-24">
          ...
          <p className="modal-sub">
            <span style={{ color: "var(--gold)", fontWeight: 700 }}>神 {ghostUserDisplay.github_username}</span>
            {youWin ? " に " : tie ? " と " : " に "}
...
### `apps/api/src/controller/play-session/start-challenge-gods.ts` / `apps/api/src/service/play-session-service.ts`

実装済みのため削除（「Phase 4 完成までは 409 を返す」コメントは既に整理済み）。
