...
- トップ 10 がまだ存在しない / 全員のキーストロークログが取得不能な場合は、クライアント側は disabled せず、Server Action のエラーレスポンスでエラーメッセージを表示して通常モードへ誘導する。
- ゴーストは `publicRanking=true` のユーザーのみが候補。`publicRanking=false` のユーザーはそもそもトップ 10 に入らないため、神々候補にも含まれない。
- API レスポンスは `github_username` + `grade` を別フィールドで返し、UI 側で `@{github_username}` + グレード行で表示する（神々はオールタイムトップ 10 に入る実力者なので、ほぼ Staff Engineer 以上の表示になる想定）。
...
- 常に 20 問固定（schema が `.length(20)` で 20 問を強制）。20 問未満しか持たないゴーストは候補から除外される。
...
- MVP では retention 未実装、全件永続化（保存期間制御 / 入賞プレイ別扱いは [拡張余地](#拡張余地) に記載）。
...
- 候補が尽きるまでサーバー側で全候補を試行し、全滅した場合は HTTP 409 を返す。クライアント側で再試行はしない。
...
- **モバイル対応**：MVP では PC のみ
- **keystroke_logs の retention 制御**：MVP では全件永続化。将来的にはプレイ後 365 日でパージし、オールタイムトップ 10 入賞プレイのみ永続保存とする想定
...
| メソッド | パス | 説明 |
| --- | --- | --- |
| POST | `/api/play-sessions/challenge-gods` | 認証済みユーザー用。`{ languageId }` を渡すと、サーバーがランダムに神を選定し、神の出題シーケンス 20 問、`ghostSessionId`、`ghost_user_display`（`github_username` + `grade`）、**神が打った repo の `repoInfo`**、および **`ghost_keystroke_logs`（神のキーストロークログ）** をまとめて返す。トップ 10 不在 / 全候補で keystroke log 取得不能の場合は HTTP 409 |
| POST | `/api/play-sessions/guest/challenge-gods` | ゲスト用ステートレス endpoint。レスポンス内容は認証版と同じだが **`session_id` を持たず**、Redis state も作らない。`/finish` 相当の保存処理もスキップする |
...
| テーブル | 主要カラム | 説明 |
| --- | --- | --- |
| `keystroke_logs` | `playSessionId(PK)`, `compressedLog(bytea)`, `createdAt`, `updatedAt` | キーストロークログ本体（gzip） |

- `play_sessions` の `mode` カラムが `challenge_gods` のとき `ghostSessionId` を保持。
- 通常モードでも `keystroke_logs` は記録する（将来の神候補となるため）。
- MVP では retention 未実装、全件永続化（保存期間制御は [拡張余地](#拡張余地) 参照）。
...
sequenceDiagram
    ...
    alt トップ 10 不在 / 全候補で keystroke log 取得不能
        API-->>W: HTTP 409
        W-->>U: エラーメッセージ表示 + 通常プレイ誘導
    else トップ 10 存在
        API->>DB: 該当言語のオールタイムトップ 10 を取得
        API->>API: ランダムで 1 人選定 → ghostSessionId
        API->>DB: ghost の play_session_problems 20 問 + ghost セッションの crawled_repos + keystroke_logs を取得
        API->>Redis: crawledRepoId / languageId / mode=challenge_gods / ghostSessionId / problemIds / userId を TTL 5 分で保存
        API-->>W: { sessionId, problems: [...20], ghostSessionId, ghostUserDisplay, repoInfo, ghostKeystrokeLogs }

        W->>W: 120 秒タイマー開始 + 神のキーストロークログ（レスポンス同梱）再生スタート
        ...
        alt 認証済み
            ...
            API->>DB: user_language_best から ORDER BY score で最新順位を取得
            API-->>W: { rank, total_ranked_players, best_score, best_accuracy, best_played_at, best_play_session_id, grade, next_grade, language }
        end
