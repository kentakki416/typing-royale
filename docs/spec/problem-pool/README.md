...
| `disabled_reason` の実装値 | `"too_few_problems"` / `"license_not_allowed"` / `"license_changed"` / `"fetch_timeout"` | 採用条件不足 / ライセンス NG / 月次再検証で NG 化 / GitHub API fetch timeout |
...
1. `languageId` に紐づく `crawled_repos` から `disabled=false` の repo を抽出（disabled でない repo は採用条件を満たして `storedCount > 0` が保証される）
2. その中からランダムに 1 repo を選定（**クールダウン等は行わない、純粋ランダム**）
3. その repo の `problems` から **20 問をランダム抽出**
4. **20 問未満の場合は 404 を返す（fallback 補填は未実装）**。将来的に「他 repo から不足分を補填」を入れる余地は残してあるが、現状の MVP 実装では行っていない。
5. レスポンスに `repoInfo`（後述）と 20 問を同梱

ゲスト経路 `POST /api/play-sessions/guest/solo` も上記と同じ抽選ロジック（同じ内部関数）を共有する。
...
```ts
type RepoInfo = {
  description: string | null  // "A type-safe schema validation library"（null の場合あり）
...
（`fallback` フィールドは現在の実装では存在しない。20 問補填が未実装のため。`repo` ではなく `name` キーで返す。）
...
- **スケジュール**：毎週月曜 03:00 JST。GitHub API の負荷が比較的低い時間帯。
  - 注：このスケジュール宣言の実体は EventBridge 側（AWS インフラ）にあり、本リポジトリ内（`apps/cron`）には cron 表記は含まれない。task は単発実行 CLI として実装し、EventBridge → ECS Scheduled Task が定期起動する。
- **実行環境**：API サーバとは別プロセス（`apps/cron` パッケージ内の `pnpm crawler:run:<slug>` CLI として実装、言語ごとに別 task）。本番は AWS EventBridge → ECS Scheduled Task で起動。
  - **実装状況**：現状は TypeScript task（`crawler-run-typescript`）のみ実装済み。JavaScript task は未実装（将来追加）。
...
- HTTP 5xx・レート制限は **`lib/retry.ts` の指数バックオフ（最大 3 回、base 1s, factor 2, jitter ±20%）** で吸収。3 回失敗時は `crawler_run_items` に `status="failed"` + `error` を記録し、`CrawledRepo` は登録せず次回 run で再試行可能なまま残す（`disabled=true` で固定化はしない）。
...
> リプレイ用に問題詳細だけを取りに行く `GET /api/problems/:id` は **存在しない**。リプレイ画面は `GET /api/replays/:playSessionId` のレスポンスに同梱される `problems[]` を使う（`sourceUrl` も同レスポンス内に含まれる）。
...
| `crawled_repos` | `id`, `languageId`, `githubId(unique)`, `owner`, `name`, `fullName`, `stars`, `license`, `defaultBranch`, `commitSha`, `description`, `homepage(nullable)`, `topics(jsonb)`, `candidatesCount`, `storedCount`, `disabled`, `disabledReason(nullable)`, `crawledAt` | 処理済み repo（成功 / 不適格 / 失敗 すべて）。`disabled=false` の repo が `/solo` の抽選対象（disabled でない repo は採用条件を満たして `storedCount > 0` が保証されるため、`storedCount > 0` の追加フィルタは抽選クエリ側では掛けていない）。`candidatesCount` は採用条件を満たした総数、`storedCount` は実際に保存された件数（≤ 100）。`disabled=true` は失敗 / 採用候補不足 / ライセンス変更 / fetch timeout 等で除外された repo |
...
```mermaid
flowchart TD
    Solo["POST /api/play-sessions/solo<br/>POST /api/play-sessions/guest/solo"] --> Pick[disabled=false の repo から<br/>1 つランダム抽選]
    Pick --> Pool[(その repo の problems から<br/>20 問 random 抽出)]
    Pool --> Fallback{20 問取れた?}
    Fallback -->|Yes| Out[Problem 20 問 + repoInfo を返却]
    Fallback -->|No| Err[404 を返す<br/>fallback 補填は未実装]
