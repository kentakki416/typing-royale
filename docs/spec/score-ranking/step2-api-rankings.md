...
| 冪等性 | 冪等 | 冪等 |
| 呼び出し元 | apps/web の `/hall-of-fame`（全期間 TOP 10）、`/`（既存 placeholder の差し替え）。`/ranking` ページは月間版（`/api/rankings/monthly`）を表示 | apps/web の `/play/[sessionId]` リザルト画面（step6）、`/mypage`（step6） |
...
> 補足: 同じ `ranking-service` には `/api/rankings/monthly` 用の `listMonthly` も export 済み（`/ranking` ページが消費する月間ランキング API）。
