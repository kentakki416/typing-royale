...
`/api/play-sessions/*` はゲスト（未ログイン）でもアクセス可能。Redis の `PlaySessionState.userId` を `number | null` にし、`/finish` 時に `userId === null` なら DB 書き込み・ランキング更新・rewards 生成を **すべてスキップ** する。レスポンスでは `persisted: false` でゲストを識別し、`new_rank` / `top_ten_boundary_score` / `grade_up` はすべて `null`、`best_score_updated` は `false` を返す。
