...
- 1 セッション = **初期 120 秒**。寿司打の「お勧めコース」と同様の体験。
- ただし **combo マイルストーン到達時にセッション時間が動的延長される**（詳細は [`../combo-time-bonus/README.md`](../combo-time-bonus/README.md) 参照）。120 秒固定ではない。
- 開始時にカウントダウンが始まり、終了時にプレイ終了 → リザルト画面へ。
- 関数を 1 つ打ち終わったら **自動的に次の関数が出題** され、残り時間が尽きるまで連続で続く。
- セッション終了時点で関数の途中だった場合、そこまでに **正しく入力できた文字数を含めて** 集計する。
...
| **ゲスト** | **DB には一切保存しない**。リザルト画面でサーバー集計したスコアを表示するのみ | 何も保存しない |

#### API 経路の分離（認証必須 vs ゲスト）

認証状態によって叩く endpoint そのものを分離している。Service / Controller / schema が **1 つの責務だけを持つ** ように保つため。

| 用途 | 認証必須経路 | ゲスト用ステートレス経路 |
| --- | --- | --- |
| 通常モード開始 | `POST /api/play-sessions/solo` | `POST /api/play-sessions/guest/solo` |
| 神々モード開始 | `POST /api/play-sessions/challenge-gods` | `POST /api/play-sessions/guest/challenge-gods` |
| 結果集計 | `POST /api/play-sessions/:id/finish` | `POST /api/play-sessions/guest/finish` |

- 認証必須経路は従来通り Redis に `PlaySessionState` を保存し `/finish` 時に DB へ atomic 書き込み
- ゲスト用経路は **Redis を使わない / DB に書き込まない**。`/guest/solo` `/guest/challenge-gods` は問題抽選結果と `repo_info` をそのまま返すだけ（`session_id` も発行しない）。`/guest/finish` は body に `problem_ids` を直接含めて受け取り、サーバーで codeBlock を引いて mistypeStats と score を再集計するだけ
- 物理限界チェック（`typedChars × accuracy` の上限 1500 文字）はゲストでも適用される。シェア時の不正スコア防止のため

#### Web 側の振り分け

`apps/web` の Server Action (`startPlaySession`) が **ログイン状態を見て叩き分ける**。クライアントは沈黙のまま適切な endpoint を選ぶ。ゲスト用 `/finish` は body に `problem_ids` を載せる必要があるため、PlayLoop は `/guest/solo` のレスポンスを sessionStorage に保持する。

#### リザルト画面の表示分岐

ゲスト用 `/finish` のレスポンスは `persisted` / `best_score_updated` / `new_rank` / `top_ten_boundary_score` / `grade_up` を持たない。Web 側の PlayLoop が認証必須版のレスポンス形に正規化（これらを `false` / `null` 充填）して ResultScreen に渡し、ResultScreen は `persisted === false` をゲスト判定に使う。

ゲストのリザルト画面では以下のように案内する：

- 「💾 このスコアは保存されていません」カードを表示
- 「GitHub で記録を残す」ボタン → `/sign-in` へ誘導
- 「全期間ランキング」「エンジニアグレード」「TOP 10 入り」「達成カード昇格」などログインユーザー向けの表示はすべて非表示

MVP では **ログイン後にゲスト時の直前スコアを引き継ぐ機能は実装しない**。ログイン後の次回プレイから記録が残る、というシンプルな運用にする。これにより：

- 「離脱の山」も DB を汚さない
- 引き継ぎ用の IndexedDB / claim API といった複雑度を持たずに済む（OAuth state 経由のセッション ID 受け渡し等が不要）
- ゲストプレイデータがブラウザに残り続けない（プライバシー観点でも望ましい）

### リザルト画面に表示する順位

- **リアルタイムで `user_language_best` を集計して順位を返す**（snapshot / バッチ集計は採用していない）。
- 圏外しきい値（1000 位以内のみ表示等）は無く、**ランキング登録対象なら必ず順位が出る**。レスポンスには `rank` と `total_ranked_players` を含める。
- 表示する順位は、当該プレイヤーの **言語別 × 期間別（全期間 / 月間）の現在順位**。
...
- **保存内容**：`sessionId`, `userId`（**number 必須**）, `languageId`, `mode`, `ghostSessionId(nullable)`, `crawledRepoId`, `出題シーケンス（20 問の問題 ID 配列）`
...
| 画面 | 概要 |
| --- | --- |
| 言語選択 | TypeScript / JavaScript を選ぶ。同画面に「通常プレイ」と「神々に挑戦」の 2 ボタンを配置。**MVP 期間は JavaScript が `comingSoon: true` で disabled** |
...
| リザルト画面 | 累計文字数・スコア・正確率・出題数 / 完走数・**現在のエンジニアグレードと次グレードまでの進捗**（即時表示）・**リアルタイム集計の全期間順位 / `total_ranked_players`**（ランキング登録対象なら必ず順位が出る、圏外しきい値は無い）・**よく間違える文字（上位 5〜10）**・**ちなみに今回のリポジトリは XXX… コメント**（`repoInfo` の description / homepage / topics を使用）・特典獲得通知（グレードアップ時は特大演出）・シェアボタン |

## 必要な API

| メソッド | パス | 説明 |
| --- | --- | --- |
| POST | `/api/play-sessions/solo` | **通常モード**のセッション開始（認証必須）。Body：`{ languageId }`。サーバーが該当言語の `eligible=true` repo から **1 つを純粋ランダム抽選** し、その repo の関数から 20 問を抽出してレスポンスに同梱。Redis にステート（sessionId / userId / 出題シーケンス / `mode=solo` / `crawledRepoId`）を TTL 5 分で保持。レスポンス（snake_case）：`{ session_id, problems: [...20], repo_info }`（`repo_info` の型は [`../problem-pool/README.md`](../problem-pool/README.md) 参照） |
| POST | `/api/play-sessions/challenge-gods` | **「神々に挑戦」モード**のセッション開始（認証必須）。Body：`{ languageId }`。該当言語のオールタイムトップ 10 からランダム 1 人（自分を除く）を選定し、その人の出題シーケンス先頭 20 問を抽出。神が打った repo の `repo_info` も継承して同梱。Redis に `mode=challenge_gods` / `ghostSessionId` / `userId` / `crawledRepoId` を保持。レスポンス（snake_case）：`{ session_id, problems: [...20], ghost_session_id, ghost_user_display, ghost_keystroke_logs, repo_info }`。トップ 10 不在時は HTTP 409 を返し、クライアントはボタン無効化＋ソロ誘導 |
| POST | `/api/play-sessions/:id/finish` | プレイ終了時の結果集計（認証必須）。セッション時間は combo time bonus で動的延長されるため厳密に 120 秒固定ではない。クライアントから `{ typed_chars, accuracy, keystroke_logs }` を受け取り、**サーバーで `score = typed_chars × accuracy` を計算**。**人間の物理限界（120 秒で 1500 文字）を超えるスコアは HTTP 400 で拒否**し DB に書かない。DB へ atomic 書き込み（`play_sessions` / `play_session_problems` / `keystroke_logs` / `user_lifetime_stats` / `user_language_best`）。サーバー側で誤打鍵を文字単位で集計し `mistype_stats` を生成。`best_score` 更新時はグレード再判定（[`../score-ranking/README.md` 「グレード判定の実装」](../score-ranking/README.md#グレード判定の実装)）。レスポンス：`{ score, typed_chars, accuracy, problems_played, problems_completed, mistype_stats, persisted: true, best_score_updated, grade_up, new_rank, top_ten_boundary_score, total_ranked_players, monthly_top_ten_boundary_score }` |
| POST | `/api/play-sessions/guest/solo` | **ゲスト用**通常モードのセッション開始（認証不要・ステートレス）。Body：`{ languageId }`。問題抽選結果と `repo_info` をそのまま返すだけで Redis 保存なし、`session_id` も発行しない。レスポンス：`{ problems: [...20], repo_info }` |
| POST | `/api/play-sessions/guest/challenge-gods` | **ゲスト用**神々モードのセッション開始（認証不要・ステートレス）。Body：`{ languageId }`。トップ 10 からランダム 1 人を選定（自分の存在概念がないので除外なし）。Redis 保存なし。レスポンス：`{ problems: [...20], ghost_session_id, ghost_user_display, ghost_keystroke_logs, repo_info }`。トップ 10 不在は 409 |
| POST | `/api/play-sessions/guest/finish` | **ゲスト用**結果集計（認証不要・ステートレス）。Body：`{ typed_chars, accuracy, keystroke_logs, problem_ids }`。Redis 不使用なので `problem_ids` をクライアントから受け取り DB から codeBlock を引いて `mistype_stats` を集計する。物理限界チェックは適用、DB 書き込みは一切なし。レスポンスにはゲストでも順位算出を行うため `new_rank` / `total_ranked_players` を含む：`{ score, typed_chars, accuracy, problems_played, problems_completed, mistype_stats, new_rank, total_ranked_players }`（`persisted=false` 系のフィールドは持たない） |
| GET | `/api/rankings/me` | リザルト画面用。query は `language=ts` 等の言語 slug を指定し、`user_language_best` のリアルタイム集計から `{ rank, total_ranked_players }` 等を返す。ゲストの呼び出しはランキング非対象なので `null` を返す |
...
| テーブル | 主要カラム | 説明 |
| --- | --- | --- |
| `play_sessions` | `id`, `user_id`（NOT NULL）, `language_id`, `mode(solo/challenge_gods)`, `ghost_session_id(nullable)`, `crawled_repo_id(FK)`, `typed_chars`, `accuracy`, `score`, `problems_played`, `problems_completed`, `mistype_stats(jsonb)`, `played_at` | プレイ結果 1 件。`crawled_repo_id` はそのセッションのメイン repo（神々モードは神が打った repo を継承）。`played_at` は `/finish` のサーバー時刻。ゲストプレイは DB に書かないため `user_id` は NOT NULL |
| `play_session_problems` | `id`, `play_session_id`, `problem_id`, `order_index`, `chars_typed`, `completed(bool)` | セッション中に出題された問題のシーケンス |
| `keystroke_logs` | `play_session_id(PK)`, `compressed_log(bytea)`, `created_at`, `updated_at` | キーストロークログ（ゴースト・リプレイで利用） |
| `user_lifetime_stats` | `user_id(PK)`, `total_sessions`, `total_typed_chars`, `best_score`, `best_score_by_language(jsonb)`, `lifetime_mistype_stats(jsonb)`, `current_grade`, `current_grade_reached_at` | プレイヤー生涯統計。`/finish` で upsert される |
| `user_language_best` | `user_id`, `language_id`, `best_score`, `best_play_session_id`, `played_at` | 言語別ベストスコアの正本。ランキング順位はこのテーブルを **リアルタイム集計** して算出（snapshot バッチは無い） |
...
sequenceDiagram
    ...
    alt ログインユーザー
        ...
        API->>DB: その repo の problems から 20 問抽出
        API->>Redis: sessionId / 出題シーケンス / userId / mode=solo / repoIdentifier を TTL 5 分で保存
        API-->>W: { sessionId, problems: [...20], repoInfo }
    else ゲスト (未ログイン)
        W->>API: POST /api/play-sessions/guest/solo { languageId }
        API->>DB: 該当言語の eligible=true repo から 1 つランダム抽選
        API->>DB: その repo の problems から 20 問抽出
        Note over API,Redis: Redis 保存なし / sessionId も発行しない
        API-->>W: { problems: [...20], repoInfo }
        W->>W: クライアント側で UUID を発行（ルーティング用）
    end
    ...
    W->>W: タイマー終了（combo マイルストーン到達でセッション時間延長され得る）
    alt ログインユーザー
        W->>API: POST /api/play-sessions/:id/finish { typed_chars, accuracy, keystroke_logs }
        API->>Redis: ステート取得（出題シーケンス + userId + crawledRepoId）
        API->>API: score = typed_chars × accuracy をサーバーで計算<br/>上限超え（120秒で1500文字超）なら HTTP 400 で拒否<br/>クライアントの is_correct は信用せず<br/>サーバー側で input_char === expected を再判定
        API->>DB: play_sessions / play_session_problems / keystroke_logs / user_lifetime_stats / user_language_best を保存
        API->>DB: user_language_best からランキングをリアルタイム集計<br/>(rank / total_ranked_players / top_ten_boundary_score 等)
        API->>Redis: ステート削除
        API-->>W: { score, ..., persisted: true, best_score_updated, grade_up, new_rank, top_ten_boundary_score, total_ranked_players, monthly_top_ten_boundary_score }
        W->>API: GET /api/rankings/me?language=ts
        API->>DB: user_language_best をリアルタイム集計
        API-->>W: { rank, total_ranked_players, ... }
    else ゲスト (未ログイン)
        W->>API: POST /api/play-sessions/guest/finish { typed_chars, accuracy, keystroke_logs, problem_ids }
        API->>API: score を計算 + 物理限界チェック<br/>input_char === expected をサーバー側で再判定
        API->>DB: problem_ids から codeBlock を取得（mistype_stats 集計用、書き込みなし）
        API->>DB: user_language_best からランキングを参照（ゲストでも順位は返す）
        Note over API,DB: DB には一切書き込まない
        API-->>W: { score, typed_chars, accuracy, mistype_stats, problems_played, problems_completed, new_rank, total_ranked_players }
        W->>W: Web 側で persisted=false 等の null/false 値を補って結果画面に渡す
    end
