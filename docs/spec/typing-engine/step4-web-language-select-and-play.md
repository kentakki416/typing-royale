...
step3 で実装した `/finish` を 120 秒タイマー終了時に呼ぶ部分も本 step に含める（API 側の作業は step3 で完了済み、ここはクライアント呼び出しのみ）。本 step では認証済みユーザーを主対象とするが、**同 step でゲスト分岐（`/api/play-sessions/guest/*` 経路）も追加済み**：Server Action が未ログイン時はゲスト用 endpoint を叩き、PlayLoop は `/guest/finish` に `problem_ids` を載せる構成。
...
| Route | コンポーネント | 概要 |
|---|---|---|
| `/play` | Server + Client | 言語選択画面（TypeScript / JavaScript × 通常 / 神々） |
...
| `playing` | カウントダウン + 関数本体 + cursor。タイマーは `useCountdown` フックで rAF + `performance.now()` 差分計算、combo マイルストーン到達時に `extendDuration(ms)` で動的延長サポート | スプラッシュ 2 秒経過 |
| `result` | リザルト UI（step5） | セッション時間経過 OR 20 問全完走 → `/finish` 完了 |
...
- **改行 + 行頭 whitespace の自動スキップ**：`\n` を正しく打鍵した直後、次の行の行頭 whitespace（インデント spaces / tabs）はカーソルを自動的にスキップさせる。`Shift+Enter` を押した場合は、現在行内の whitespace を行末まで一括スキップする（コードのインデント部分で手動打鍵を強要しない）
