...
- **MVP では全 play_session を保持** し、`/api/replays/:playSessionId` で個別に閲覧可能（`play_sessions.persistReplay` カラムは未導入）。
- 1 リプレイ = 1 セッション（120 秒 / 複数問題を含む）。
- 期間切替によりトップ 10 から押し出されたプレイも DB 上は保持される（cron / cleanup 未実装）。
- プレイヤーが `publicRanking=false` の場合、**API 側で 404 を返すことで非公開を実現**。後から `publicRanking` を OFF に切り替えたユーザーの過去リプレイも同じく 404（DB 上のレコードは残るが閲覧不可）。
...
- リプレイデータは不変なので CDN キャッシュ TTL を長く設定可（例：7 日）（MVP では未設定、都度 DB）。
- `GET /api/replays/:playSessionId` は最初のリクエストで DB から構築 → CDN にキャッシュ → 以降は CDN ヒット（MVP では未設定、都度 DB）。
...
| プレイヤー詳細ページ | そのプレイヤーの代表的なリプレイ一覧（deferred / 未実装） |
...
| GET | `/api/replays/featured` | 注目リプレイ一覧（Hall of Fame 連携）（step2 未実装） |
