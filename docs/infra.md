...
- [各コンポーネントの責務](#各コンポーネントの責務)
  ...
  - [crawler / batch (ECS Scheduled Task)](#crawler--batch-ecs-scheduled-task)
...
| GitHub OAuth | **GitHub アプリ**（運営アカウント） | クローラ用 PAT も同アカウントで管理 |
...
flowchart TB
    ...
    subgraph AWS[AWS 単一 VPC]
        ...
        ECSCrawler[ECS Scheduled Task<br/>apps/crawler]
        ECSBatch[ECS Scheduled Task<br/>ranking 毎時バッチ]
        EventBridge[EventBridge<br/>週次 + 毎時スケジュール]
    ...
    EventBridge -->|週次月曜 03:00 JST| ECSCrawler
    EventBridge -->|毎時 00 分| ECSBatch
    ECSCrawler --> Github
    ECSCrawler --> RDS
    ECSBatch --> RDS
    ECSBatch --> Redis
...
### crawler / batch (ECS Scheduled Task)

- ホスティング：ECS Scheduled Task（API と同じ Docker イメージを別エントリポイントで起動）
- 起動：**EventBridge → ECS RunTask**
- タスクは API サービスとは別の Task Definition を持ち、エントリポイントを切り替えて起動
...
| `crawler-batch-deploy-ecs` | push to main (crawler / batch 変更時) | ECS Scheduled Task の Task Definition を更新 |
...
| `GITHUB_PAT`（運営アカウント、クローラ用） | AWS Secrets Manager | crawler のみ |
| `JWT_SECRET` | AWS Secrets Manager | api / Refresh token 署名 |
| `DATABASE_URL`（RDS） | AWS Secrets Manager + Vercel Env | api / crawler / batch |
| `REDIS_URL`（ElastiCache） | AWS Secrets Manager | api / batch |
| `SENTRY_DSN` | Vercel Env + AWS Secrets Manager | web / api / crawler / batch |
...
| 用途 | サービス |
| --- | --- |
| サーバー側エラー | **Sentry**（api / crawler / batch） |
...
| AWS ECS Scheduled Task（crawler、batch） | 短時間稼働、1 タスク数分 | ~$2 |
