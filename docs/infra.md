...
- ENV：`NEXT_PUBLIC_API_URL`、`GITHUB_CLIENT_ID/SECRET`、Vercel Analytics token
...
- ENV：`DATABASE_URL`（RDS）、`REDIS_URL`（ElastiCache）、`JWT_SECRET`、`S3_BUCKET`
...
| 用途 | サービス |
| --- | --- |
| クライアント側 Web Vitals | **Vercel Analytics** |
...
| アラート | CloudWatch → SNS → Slack |

エラー監視（旧 Sentry 想定）は MVP では持たない。`logger.error` の出力を CloudWatch Logs Insights で検索する運用とし、必要になった段階で別途検討する。
